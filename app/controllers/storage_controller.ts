...
export default class StorageController {
  ...
  async show(context: HttpContext) {
    ...
    do {
      ...
      const response = await fetch(url, {
        headers: { authorization: `Bearer ${blobApiToken}` },
    ...
    if (!response.ok) {
      throw new Error(`Cannot get \`blob-api\` storage breakdown: HTTP ${response.status}`)
    }
  ...
  private async ensureSeeded(
    ...
    const count = await Blob.query().where('creator', did).count('* as count').firstOrFail()

    // Assumption here locally is that there should be a blob if seeded.
    if (Number(count.$extras.count) > 0) return
      ...
      await Promise.all(
        slice.map(async function (cid) {
          try {
            ...
            await Blob.firstOrCreate(
              { cid, creator: did },
              { cid, createdAt: new Date(0).toISOString(), creator: did, mimeType: type, size }
            )
          } catch (error) {
